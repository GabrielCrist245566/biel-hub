if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Biel Hub is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Biel Hub is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Biel Hub FE v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Biel Hub Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Biel Hub support server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.com/invite/5TmmPexPkP')
		notify('Discord Invite', 'Copied to clipboard!\nhttps://discord.gg/5TmmPexPkP')
	else
		notify('Discord Invite', 'discord.gg/5TmmPexPkP')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Biel Hub may have issues or features that are not functioning correctly.") end
end)
